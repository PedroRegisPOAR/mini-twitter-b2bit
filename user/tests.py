...
class UserViewSetTestCase(BaseTestCase):
    def test_create_1(self):
        ...
        Quando:
          - for feita a requisição de criação de um usuário
            `POST /api/user/`
        Então:
          - a resposta deve ser HTTP_201_CREATED
    ...
    def test_create_2(self):
        ...
        Quando:
          - for feita a requisição de criação de um usuário
            `POST /api/user/`
        Então:
          - a resposta deve ser HTTP_400_BAD_REQUEST
        """
        # TODO: why the UnicodeUsernameValidator is not working?
        data = dict(
            username="",
            email="foo@bar.com",
            name="Foo",
    ...
    def test_followers_1(self):
        ...
        Quando
          - for feita a requisição de criação de um usuário
            `POST /api/user/{user1.id}/followers/`
        Então:
          - a resposta deve ser HTTP_201_CREATED
          - a chave id deve está na resposta no campo results
        """
        data1 = dict(
            username="foobar1",
            email="foo@bar.com",
        ...
        f = Follower.objects.create(following=user1, follower=user2)
